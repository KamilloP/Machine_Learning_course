...
# def start_monitor(wandb_logger, interval=2):
...
#                 wandb_logger.log({
#                     f"gpu{local_rank}/mem_allocated_GB": gpu_mem,
#                     f"gpu{local_rank}/mem_reserved_GB": gpu_reserved,
#                 })
...
def count_fsdp_params(model):
    ...
    for p in model.parameters():
        if hasattr(p, "_local_shard"):
            ...
            # fallback (np. jeśli coś nie zostało shardowane)
...
def train_model(config, device, wandb_logger):
    ...
    # stop_event, monitor_thread = start_monitor(wandb_logger) # Each node logs monitor GPU
    # CHANGE END
    ...
    for i, batch in zip(range(config.train_steps), dataloader):
        ...
        loss = mask_loss.mean()
        if i == 0:
            # Check if model has been sharded
...
def main():
    ...
    # parser.add_argument("dropout", type=int, help="Dropout") # TODO: add it
